...
app.get("/order/create_O", async(req, res) => {
	try {
        const customers = await axios.get(base_url + "/customers");
        const products = await axios.get(base_url + "/products");
		res.render("order/create_O", { customers: customers.data, products: products.data});
	} catch (err) {
		console.error(err);
		res.status(500).send("Error");
	}
...
app.get("/update_O/:id", async (req, res) => {
	try {
		...
		res.render("order/update_O", { order: response.data, customers: customers.data, products: products.data});
	} catch (err) {
		console.error(err);
		res.status(500).send("Error");
	}
